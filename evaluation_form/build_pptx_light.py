"""
build_pptx_light.py  —  MindGuard Thesis Defence Presentation Generator (Light Theme)
"""
...
C_BG        = RGBColor(0xF8, 0xFA, 0xFC)   # light grey background
C_CARD      = RGBColor(0xFF, 0xFF, 0xFF)   # white card
C_PRIMARY   = RGBColor(0x4F, 0x46, 0xE5)   # indigo
C_SECONDARY = RGBColor(0x8B, 0x5C, 0xF6)   # violet
C_ACCENT    = RGBColor(0x05, 0x96, 0x69)   # emerald
C_DANGER    = RGBColor(0xDC, 0x26, 0x26)   # red
C_WARNING   = RGBColor(0xD9, 0x77, 0x06)   # amber
C_TEXT      = RGBColor(0x33, 0x41, 0x55)   # dark slate
C_MUTED     = RGBColor(0x64, 0x74, 0x8B)   # slate grey
C_DARK      = RGBColor(0x0F, 0x17, 0x2A)   # very dark blue/black (replaces C_WHITE for text)
C_WHITE     = RGBColor(0xFF, 0xFF, 0xFF)   # actual white
...
def card_box(slide, l, t, w, h, icon, title, body, icon_size=22, title_size=12, body_size=10.5):
    add_rect(slide, l, t, w, h, C_CARD, RGBColor(0xE2, 0xE8, 0xF0), Pt(0.5))
    # icon box
    add_rect(slide, l+Inches(0.15), t+Inches(0.12), Inches(0.5), Inches(0.45), C_PRIMARY)
    add_textbox(slide, icon, l+Inches(0.15), t+Inches(0.12), Inches(0.5), Inches(0.45),
                font_size=icon_size, color=C_WHITE, align=PP_ALIGN.CENTER)
    # title
    add_textbox(slide, title, l+Inches(0.15), t+Inches(0.62), w-Inches(0.3), Inches(0.28),
                font_size=title_size, bold=True, color=C_DARK)
    # body
    tb = slide.shapes.add_textbox(l+Inches(0.15), t+Inches(0.92), w-Inches(0.3), h-Inches(1.05))
    tf = tb.text_frame
    tf.word_wrap = True
    p = tf.paragraphs[0]
...
add_rect(sl, 0, 0, SLIDE_W, Inches(3.5), RGBColor(0xF1, 0xF5, 0xF9))
add_rect(sl, 0, Inches(3.5), SLIDE_W, Inches(4.0), RGBColor(0xE2, 0xE8, 0xF0))

# Badge
badge = add_rect(sl, Inches(4.5), Inches(1.2), Inches(4.3), Inches(0.3), RGBColor(0xEE, 0xF2, 0xFF), C_PRIMARY, Pt(0.5))
...
add_textbox(sl, "🧠🛡️", Inches(5.8), Inches(1.62), Inches(2), Inches(0.7),
            font_size=30, align=PP_ALIGN.CENTER, color=C_DARK)

# Title
add_textbox(sl, "MindGuard", Inches(1.5), Inches(2.45), Inches(10.3), Inches(1.0),
            font_size=48, bold=True, color=C_DARK, align=PP_ALIGN.CENTER)
...
add_textbox(sl,
    ...
    font_size=13, color=C_TEXT, align=PP_ALIGN.CENTER)
...
# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 2 — SCENARIO (NEW)
# ══════════════════════════════════════════════════════════════════════════════
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Scenario", [("Meet ", C_DARK), ("Sarah", C_PRIMARY)])
add_textbox(sl, "02", Inches(12.5), Inches(0.15), Inches(0.7), Inches(0.3),
            font_size=10, color=C_PRIMARY, bold=True)

# Story text
add_textbox(sl, "Sarah is a 15-year-old high school student.", Inches(0.5), Inches(1.2), Inches(12.3), Inches(0.4),
            font_size=16, bold=True, color=C_DARK)

add_textbox(sl,
    "She has been feeling increasingly isolated over the past few weeks. Instead of talking to her parents or teachers, she logs into her social media accounts late at night and writes:",
    Inches(0.5), Inches(1.7), Inches(12.3), Inches(0.7),
    font_size=12, color=C_TEXT, wrap=True)

# Social Media Mockup
add_rect(sl, Inches(2.0), Inches(2.6), Inches(9.3), Inches(2.5), C_CARD, RGBColor(0xE2,0xE8,0xF0), Pt(1.0))
# Header
add_rect(sl, Inches(2.2), Inches(2.8), Inches(0.4), Inches(0.4), RGBColor(0xE2,0xE8,0xF0)) # Profile pic placeholder
add_textbox(sl, "Sarah", Inches(2.7), Inches(2.8), Inches(2.0), Inches(0.2), font_size=12, bold=True, color=C_DARK)
add_textbox(sl, "@sarah_smiles · 2 hours ago", Inches(2.7), Inches(3.0), Inches(2.0), Inches(0.2), font_size=10, color=C_MUTED)

# Post content
add_textbox(sl,
    "\"It feels like everyone else has it together but I'm just drowning. Every day is so exhausting and I don't see the point in trying anymore. Nobody would even notice if I just stopped.\"",
    Inches(2.2), Inches(3.4), Inches(8.9), Inches(1.2),
    font_size=14, italic=True, color=C_DARK, wrap=True)

# The gap
add_rect(sl, Inches(0.5), Inches(5.4), Inches(12.3), Inches(1.4), RGBColor(0xFE,0xF2,0xF2), C_DANGER, Pt(0.5))
add_textbox(sl, "The Problem:", Inches(0.7), Inches(5.5), Inches(11.9), Inches(0.3), font_size=12, bold=True, color=C_DANGER)
add_textbox(sl,
    "Her parents are completely unaware. Traditional mental health apps on her phone require her to actively open them and journal her feelings — which she has no motivation to do.\n\nWithout passive detection, Sarah's cry for help goes entirely unnoticed.",
    Inches(0.7), Inches(5.8), Inches(11.9), Inches(0.9),
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Introduction", [("Background to the ", C_DARK), ("Field", C_PRIMARY)])
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Literature Review", [("Key Works ", C_DARK), ("Reviewed", C_PRIMARY)])
...
for i, h in enumerate(headers):
    xl = tl + sum(col_ws[:i])
    add_rect(sl, xl, tt, col_ws[i], th, RGBColor(0xEE, 0xF2, 0xFF), C_PRIMARY, Pt(0.3))
    add_textbox(sl, h, xl+Inches(0.06), tt+Inches(0.06),
                col_ws[i]-Inches(0.12), th-Inches(0.1),
                font_size=8.5, bold=True, color=C_PRIMARY)
...
rows = [
    ...
    ["Zhang et al. (2023)", "LLM emotional support for mental health","Fine-tuned LLM; overall F1 = 0.68",          "No guardian escalation; low F1",                "✔ Guardian dashboard + 94.1% High-Risk F1"],
    ["Fitzpatrick et al. (2022)","Automated CBT chatbot for depression", "Rule-based NLP; brief daily conversations",  "Requires active engagement; no passive mode",   "✔ Zero user effort required"],
...
for ri, row in enumerate(rows):
    ry = tt + th + ri * row_h
    bg = RGBColor(0xF8, 0xFA, 0xFC) if ri % 2 == 0 else C_CARD
    for ci, cell in enumerate(row):
        xl = tl + sum(col_ws[:ci])
        add_rect(sl, xl, ry, col_ws[ci], row_h, bg, RGBColor(0xE2, 0xE8, 0xF0), Pt(0.2))
        color = C_ACCENT if ci == 4 else C_DARK
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Statement of Problem", [("The ", C_DARK), ("Problem ", C_PRIMARY), ("We Solve", C_DARK)])
...
for i, (num, title, body, cite) in enumerate(problems):
    ...
    add_rect(sl, Inches(0.5), y, Inches(12.3), bh, C_CARD, RGBColor(0xE2, 0xE8, 0xF0), Pt(0.4))
    add_rect(sl, Inches(0.5), y, Pt(4), bh, C_PRIMARY)
    # Number badge
    add_rect(sl, Inches(0.65), y+Inches(0.38), Inches(0.35), Inches(0.35),
             RGBColor(0xEE,0xF2,0xFF), C_PRIMARY, Pt(0.5))
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Aim", [("What We Set Out to ", C_DARK), ("Achieve", C_PRIMARY)])
...
add_textbox(sl, "🎯", Inches(6.1), Inches(1.65), Inches(1.1), Inches(0.7),
            font_size=32, align=PP_ALIGN.CENTER, color=C_DARK)
add_textbox(sl,
    ...
    font_size=14.5, color=C_DARK, align=PP_ALIGN.CENTER, wrap=True)
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Objectives", [("Steps Taken to ", C_DARK), ("Achieve the Aim", C_PRIMARY)])
...
for i, (num, title, body) in enumerate(objectives):
    ...
    add_rect(sl, Inches(0.5), y, Inches(12.3), oh, C_CARD, RGBColor(0xE2,0xE8,0xF0), Pt(0.4))
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Methodology", [("How Each Objective Was ", C_DARK), ("Achieved", C_PRIMARY)])
add_textbox(sl, "08", Inches(12.5), Inches(0.15), Inches(0.7), Inches(0.3),
            font_size=10, color=C_PRIMARY, bold=True)

# Column headers
add_rect(sl, Inches(0.5),  Inches(1.22), Inches(5.8), Inches(0.3), RGBColor(0xEE,0xF2,0xFF), C_PRIMARY, Pt(0.3))
add_textbox(sl, "OBJECTIVE", Inches(0.5), Inches(1.24), Inches(5.8), Inches(0.26),
            font_size=8, bold=True, color=C_PRIMARY, align=PP_ALIGN.CENTER)
add_rect(sl, Inches(6.5),  Inches(1.22), Inches(6.3), Inches(0.3), RGBColor(0xEC,0xFD,0xF5), C_ACCENT, Pt(0.3))
...
for i, (obj_label, obj_body, meth_body) in enumerate(meth_rows):
    ...
    add_rect(sl, Inches(6.5), y, Inches(6.3), mh,
             C_CARD, C_ACCENT, Pt(0.4))
    add_textbox(sl, meth_body, Inches(6.65), y+Inches(0.1), Inches(6.0), mh-Inches(0.18),
                font_size=10, color=C_TEXT, wrap=True)

# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 9 — RESULTS: Obj I & II (Classification Metrics)
# ══════════════════════════════════════════════════════════════════════════════
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Results", [("Objective I & II — ", C_DARK), ("Model Performance", C_PRIMARY)])
...
for ci, h in enumerate(hdr2):
    ...
    add_rect(sl, xl, th2, col_ws2[ci], row_h2, RGBColor(0xEE,0xF2,0xFF), C_PRIMARY, Pt(0.3))
...
for ri,(row,label_color,highlight) in enumerate(table_data):
    ...
    bg = RGBColor(0xFE,0xF2,0xF2) if ri==2 else (RGBColor(0xEE,0xF2,0xFF) if highlight else C_CARD)
    for ci, cell in enumerate(row):
        xl = Inches(0.5) + sum(col_ws2[:ci])
        add_rect(sl, xl, ry, col_ws2[ci], row_h2, bg, RGBColor(0xE2,0xE8,0xF0), Pt(0.2))
        color = label_color if ci==0 else (C_DANGER if ci==4 and ri==2 else C_DARK)
        font_b = (ci == 0)
...
# ══════════════════════════════════════════════════════════════════════════════
# SLIDE 10 — RESULTS: Obj III & IV (System + SUS)
# ══════════════════════════════════════════════════════════════════════════════
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Results", [("Objective III & IV — ", C_DARK), ("System & Usability", C_PRIMARY)])
...
for i,(icon,title,body) in enumerate(sys_items):
    ...
    add_rect(sl, Inches(0.5), sy, Inches(5.8), Inches(1.3), C_CARD, RGBColor(0xE2,0xE8,0xF0), Pt(0.4))
    add_rect(sl, Inches(0.5), sy, Pt(4), Inches(1.3), C_PRIMARY)
    add_textbox(sl, icon,  Inches(0.65), sy+Inches(0.12), Inches(0.5), Inches(0.45), font_size=22, color=C_DARK)
    add_textbox(sl, title, Inches(1.28), sy+Inches(0.12), Inches(4.9), Inches(0.28), font_size=11, bold=True, color=C_DARK)
    add_textbox(sl, body,  Inches(1.28), sy+Inches(0.44), Inches(4.9), Inches(0.72), font_size=9.5, color=C_TEXT, wrap=True)
...
add_rect(sl, Inches(10.0), Inches(2.32), Inches(2.6), Inches(0.32),
         RGBColor(0xEC,0xFD,0xF5), C_ACCENT, Pt(0.5))
...
# Bar track
add_rect(sl, Inches(6.8), Inches(2.85), Inches(5.8), Inches(0.22), RGBColor(0xE2,0xE8,0xF0), RGBColor(0xCB,0xD5,0xE1), Pt(0.2))
# Bar fill (69.1%)
...
add_rect(sl, Inches(6.8), Inches(3.42), Inches(5.8), Inches(1.38),
         RGBColor(0xEE,0xF2,0xFF), C_PRIMARY, Pt(0.4))
add_textbox(sl,
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Significance", [("Contribution & ", C_DARK), ("Impact", C_PRIMARY)])
...
for i,(icon,title,body) in enumerate(user_sigs):
    ...
    add_textbox(sl, icon,  Inches(0.65), y+Inches(0.12), Inches(0.55), Inches(0.5), font_size=22, color=C_DARK)
    add_textbox(sl, title, Inches(1.35), y+Inches(0.12), Inches(5.0), Inches(0.28), font_size=12, bold=True, color=C_DARK)
    add_textbox(sl, body,  Inches(1.35), y+Inches(0.44), Inches(5.0), Inches(1.1), font_size=10, color=C_TEXT, wrap=True)

for i,(icon,title,body) in enumerate(field_sigs):
    y = sh + i*(item_h+Inches(0.05))
    add_rect(sl, Inches(6.8), y, Inches(6.0), item_h, C_CARD, C_ACCENT, Pt(0.4))
    add_textbox(sl, icon,  Inches(6.95), y+Inches(0.12), Inches(0.55), Inches(0.5), font_size=22, color=C_DARK)
    add_textbox(sl, title, Inches(7.65), y+Inches(0.12), Inches(5.0), Inches(0.28), font_size=12, bold=True, color=C_DARK)
    add_textbox(sl, body,  Inches(7.65), y+Inches(0.44), Inches(5.0), Inches(1.1), font_size=10, color=C_TEXT, wrap=True)
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Conclusion", [("What Was ", C_DARK), ("Accomplished", C_PRIMARY)])
...
for i, text in enumerate(conclusions):
    ...
    add_rect(sl, Inches(0.5), y, Inches(12.3), ch2, C_CARD, RGBColor(0xE2,0xE8,0xF0), Pt(0.4))
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "References", [("Works ", C_DARK), ("Cited", C_PRIMARY), (" in This Presentation", C_DARK)])
...
sl = add_slide()
fill_bg(sl)
slide_header(sl, "Acknowledgement", [("With Sincere ", C_DARK), ("Gratitude", C_PRIMARY)])
...
for i,(icon,title,body) in enumerate(acks):
    ...
    add_rect(sl, ax, ay, aw, ah, C_CARD, RGBColor(0xE2,0xE8,0xF0), Pt(0.5))
    add_textbox(sl, icon,  ax, ay+Inches(0.2), aw, Inches(0.55), font_size=28, align=PP_ALIGN.CENTER, color=C_DARK)
    add_textbox(sl, title, ax, ay+Inches(0.82), aw, Inches(0.32), font_size=13, bold=True, color=C_DARK, align=PP_ALIGN.CENTER)
    add_textbox(sl, body,  ax+Inches(0.15), ay+Inches(1.18), aw-Inches(0.3), Inches(0.85),
                font_size=9.5, color=C_TEXT, align=PP_ALIGN.CENTER, wrap=True)
...
sl = add_slide()
fill_bg(sl)
add_textbox(sl, "15", Inches(12.5), Inches(0.15), Inches(0.7), Inches(0.3),
            font_size=10, color=C_PRIMARY, bold=True)

add_rect(sl, 0, 0, SLIDE_W, SLIDE_H, RGBColor(0xF1, 0xF5, 0xF9))
add_rect(sl, Inches(2), Inches(0.5), Inches(5), Inches(5), RGBColor(0xE2, 0xE8, 0xF0))

add_textbox(sl, "🧠🛡️", Inches(5.7), Inches(1.2), Inches(2), Inches(0.8),
            font_size=36, align=PP_ALIGN.CENTER, color=C_DARK)

add_textbox(sl, "Thank You",
            Inches(1.5), Inches(2.1), Inches(10.3), Inches(1.2),
            font_size=56, bold=True, color=C_DARK, align=PP_ALIGN.CENTER)
...
# ── Save ──────────────────────────────────────────────────────────────────────
out_path = r"evaluation_form\MindGuard_Presentation_Light.pptx"
prs.save(out_path)
